...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname correct) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;;
;; Strategy: domain knowledge (units of measure)
(define (centimeters->meters cm)
  (/ cm 100))

(check-expect (centimeters->meters 0) 0)
(check-expect (centimeters->meters 1) 0.01)
(check-expect (centimeters->meters 100) 1)
(check-expect (centimeters->meters 150) 1.5)
...
(define (feet->meters ft)
  (centimeters->meters (inches->centimeters (feet->inches ft))))
